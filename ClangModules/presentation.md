...
## Ok, we (clang, not me) will translate automatically
...
* Important for adoption and backward compatibility


???

* benefits of modules
* fall back to preprocessor in compilers - support
...
???
* describe collection of headers
* on logical structure of module
...
* Example module std covering entire C standard library
...
* All standard headers contribute
* place in correct submodule
* std.io std.lib std.math
* help compiler build std module
* help reverse - translation of headers to module imports
...
* Wildcard export
  - Re-exports all modules imported in actual header file.
* Specifies that anything included by that submodule will be automatically re-exported.

]

???
* Re export all modules imported by your module transitively
* import CoreData imports Foundation
* Just like C preprocessor
* helps backward compatibility - same as before
...
.right-column[
* Binary representation of modules is generated on as-needed basis.
* When a module is imported (even via #include):
  1. compiler spawns a second instance of itself
  ![Spawning](https://media.giphy.com/media/ejam9wdWhqg24/giphy.gif)
  2. Parses headers in that modules.
  3. Resulting Abstract Syntax Tree (AST) is then persisted into the binary representation of the module
  4. Load in translation unit
]
...
---
.right-column[
* Caching is done in module cache.
* Parsing is done only once.
* If any of the headers change, module automatically recompiles.
]
...
```
import std.io
and your translation unit defines an abs(int) // defined in stdlib.h
will create conflict
```

* If any module is imported in any translation unit, the entire top-level module is considered a part of program.
* Conflict between entity in unimported submodule and entity in current translation unit.
* Clang may inline or devirtualize based on knowledge from unimported submodules.
